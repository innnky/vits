...
class PhonePitchCalculater:
    def __init__(self):
        ...
        print("init PhonePitchCalculater")

    def get_phone_pitch(self, f0s, spec_lengths, x, x_lengths, audio_paths):
        phone_pitchs = torch.zeros_like(x).to(f0s.device).float()
        for i in range(f0s.shape[0]):
            speaker, audio_path = audio_paths[i]

            spkmap = self.audio_path_duration_map[speaker]
    ...
    def update(self, audio_paths, durations, x_lengths):
        for i in range(len(audio_paths)):
            speaker, audio_path = audio_paths[i]
            x_length = x_lengths[i]
            # print(durations[i, :x_length], durations[i, :])
            self.audio_path_duration_map[speaker][audio_path] = durations[i,0, :x_length].long()
